...
$markdown = ""
$url = "https://raw.githubusercontent.com/mozilla-platform-ops/worker-images/develop/config/win11-64-2009-alpha.yaml"
irm $url -OutFile "$ENV:TEMP\win11-64-2009-alpha.yaml"
$config = ConvertFrom-Yaml (Get-content -Path "$ENV:TEMP\win11-64-2009-alpha.yaml" -Raw)
$Date = Get-Date -Format "yyyy-MM-dd"

## Collect software
$Software = Get-InstalledSoftware | Where-Object {
    $_.displayname -ne $null -and $_.displayVersion -ne $null
}

## Not sdk stuff? idk
$a = $Software| Where-Object {$psitem.displayname -notmatch "SDK"}

[System.String] $markdown += New-MDHeader -Text "$($config.vm.tags.worker_pool_id) Configuration" -Level 1 -NoNewLine
$markdown += "`n`n"
$markdown += New-MDHeader -Text "Installed Software" -Level 2
$markdown += $Software | Sort-Object -Property DisplayName | 
Select-Object -Property DisplayName,DisplayVersion |
New-MDTable -Columns (
    [ordered]@{
        DisplayName = "left"
        DisplayVersion = "right"
    }
)

$markdown | Out-File -FilePath "Changelog.md"